...
function bouncePlayerDown() {
    const clip = THREE.AnimationClip.findByName( clips, 'Bounce-Down' );
    const action = mixer.clipAction( clip );
    action.reset();
    action.loop = THREE.LoopOnce;
    action.timeScale = 2;
...
const bounceDownButton = document.querySelector("#bounceDown");
bounceDownButton.addEventListener("click", function () {
    bouncePlayerDown();
})
